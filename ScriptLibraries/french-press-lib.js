/*jslint onevar: false, plusplus: false */
	...
	---------------
		...
		The options are:
			indent_size (default 1)					— indentation size,
			indent_char (default tab)			— character to indent with,
	...
	*/
	...
	library.beautify = function(js_source_text) {
		...
		function trim_output(eat_newlines) {
			...
			while (output.length && (output[output.length - 1] === ' ' || output[output.length - 1] === indent_string || output[output.length - 1] === preindent_string || (eat_newlines && (output[output.length - 1] === 'n' || output[output.length - 1] === 'r')))) {
				output.pop();
			}
		}
	
		function trim(s) {
			return s.replace(/^ss*|ss*$/, '');
		}
		...
		function print_newline(ignore_repeated) {
			...
			if (output[output.length - 1] !== "n" || !ignore_repeated) {
		...
		function print_single_space() {
			...
			if (last_output !== ' ' && last_output !== 'n' && last_output !== indent_string) { // prevent occassional duplicate space
				output.push(' ');
			}
		}
		...
		function get_next_token() {
			...
			if (c === '/') {
				...
				if (input.charAt(parser_pos) === '/') {
					comment = c;
					while (input.charAt(parser_pos) !== 'r' && input.charAt(parser_pos) !== 'n') {
			...
			if (c === '#') {
				...
				if (output.length === 0 && input.charAt(parser_pos) === '!') {
					// shebang
					resulting_string = c;
					while (parser_pos < input_length && c != 'n') {
						c = input.charAt(parser_pos);
						resulting_string += c;
						parser_pos += 1;
					}
					output.push(trim(resulting_string) + 'n');
					print_newline();
		...
		while (js_source_text && (js_source_text[0] === ' ' || js_source_text[0] === 't')) {
		...
		while (true) {
			...
			case 'TK_BLOCK_COMMENT':
				...
				var lines = token_text.split(/x0a|x0dx0a/);
		...
		var sweet_code = preindent_string + output.join('').replace(/[n ]+$/, '');
		return sweet_code;
